...
class _AddAddressScreenState extends State<AddAddressScreen> {
  ...
  Future<void> _addAddress() async {
    ...
    try {
      ...
      final headers = await ApiConfig.buildHeaders();
      final response = await http.post(
        Uri.parse(ApiConfig.addUserAddress),
        headers: headers,
        body: jsonEncode({
          'accountId': accountId,
      ...
      if (response.statusCode == 200) {
        ...
        if (data['msg'] == "success") {
          if (mounted) {
            // ScaffoldMessenger.of(context).showSnackBar(
            //   const SnackBar(
            //     content: Text('เพิ่มที่อยู่สำเร็จ'),
          ...
          throw Exception(data['msg'] ?? 'ไม่สามารถเพิ่มที่อยู่ได้');
        }
      } else {
            ...
            children: [
              ...
              _buildInputCard(
                ...
                child: TextFormField(
                  ...
                  maxLines: 3,
                  decoration: InputDecoration(
                    hintText: 'กรุณาใส่ที่อยู่ เช่น เลขที่ หมู่บ้าน ถนน จุดสังเกตุบริเวณใกล้บ้าน',
                    border: OutlineInputBorder(
                      borderRadius: BorderRadius.circular(8),
                      borderSide: BorderSide(color: Colors.grey[300]!),
                    ),
